...
import { Print as PrintIcon, PictureAsPdf as PdfIcon, ArrowBack } from '@mui/icons-material';
...
const TeacherViewThird = () => {
  ...
  useEffect(() => {
    ...
    setSchool({
      name: schoolData.name || 'GradLink365',
...
const handleDownloadPDF = async (student) => {
  try {
    ...
    setCurrentStudent({
      ...student,
      student_photo: photoUrl // Use the full URL here
    });
...
const PrintableResult = React.forwardRef(({ student, subjects, school, className, session, term }, ref) => (
  <div ref={ref} style={{ 
    ...
    {/* Student Info with Photo - Reduced Height */}
    <div style={{ 
      display: 'flex', 
...
<div style={{ flex: 1 }}>
  <p style={{ margin: '2px 0' }}><b>Admission No:</b> {student.admission_number}</p>
  <p style={{ margin: '2px 0' }}><strong>Position:</strong> {getOrdinalSuffix(student.position)}</p>
    ...
    <div style={{
      ...
      {/* Results Table with Serial Numbers */}
  ...
  />
      ...
      <Typography variant="h5" gutterBottom>View Third Term Results</Typography>
...
export default TeacherViewThird;